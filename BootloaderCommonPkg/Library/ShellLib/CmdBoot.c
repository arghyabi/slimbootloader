  ...
  )
{
  ...
  if (BootOption->DevType == OsBootDeviceMemory) {
    do {
      ...
      ShellPrint (L"(default '0x%x') ", CurrOption->Image[LoadImageType].RamAddress);
      Status = ShellReadUintn (Shell, Buffer, BufferSize, &IsHex);
      if (!EFI_ERROR (Status)) {
        BootOption->Image[LoadImageType].RamAddress = (UINT32) ((IsHex) ? StrHexToUintn (Buffer) : StrDecimalToUintn (Buffer));
        ShellPrint(L"Get Memory address: 0x%x\n", BootOption->Image[LoadImageType].RamAddress);
        break;
      }
